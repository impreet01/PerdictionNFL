...
function Xy(rows) {
  const X = rows.map(r => FEATS.map(k => Number(r[k])));
...
function probsLogit(model, X) {
  const { weights } = model;
  return X.map(x => 1 / (1 + Math.exp(-dot(weights, x))));
...
(async function main(){
  ...
  // Logistic
  const { X: XL, y: yL } = Xy(train);
  const logit = new LogisticRegression({ numSteps: 2500, learningRate: 5e-3 });
  logit.train(XL, yL);
  const pL_test = probsLogit(logit, Xy(test).X);
  const pL_train = probsLogit(logit, XL);

  // Decision Tree
  const cart = new CART({ maxDepth: 4, minNumSamples: 30, gainFunction: "gini" });
  const { X: XT, y: yT } = Xy(train);
  cart.train(XT, yT);
  const getProb1 = (pred) => Array.isArray(pred) ? pred[1] : Number(pred);
  const pT_test = cart.predict(Xy(test).X).map(getProb1);
  const pT_train = cart.predict(XL).map(getProb1);

  // Hybrid weight chosen on train
  const wHybrid = chooseHybridWeight(yL, pL_train, pT_train);
  ...
  writeFileSync(`${ART_DIR}/model_${TARGET_SEASON}_W${String(TARGET_WEEK).padStart(2,"0")}.json`, JSON.stringify({
    season: TARGET_SEASON, week: TARGET_WEEK, features: FEATS, logistic: logit.weights, hybrid_weight: wHybrid
  }, null, 2));
