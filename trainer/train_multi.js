...
function buildLeafFreq(cart, Xtr, ytr, alpha=2){
...
function predictTree(cart, leafStats, X){
  ...
  if (!root){
    const f=freq.get("ROOT")||{n0:0,n1:0,alpha:2}; const tot=f.n0+f.n1; const p1=(f.n1+alpha)/(tot+2*alpha)||0.5;
    return X.map(()=>p1);
  }
  return X.map(x=>{
...
function chooseHybridCV(X, y){
  // Fit logistic + tree on each train fold, score on its val fold; sweep weights
  const folds = kfoldIndices(X.length, Math.min(5, Math.max(2, Math.floor(X.length/8)))); // keep folds reasonable for ~32-64 rows
  ...
  for (const valIdx of folds){
    ...
    const cart = new CART({ maxDepth: 4, minNumSamples: 16, gainFunction: "gini" });
    cart.train(XtrS, ytr);
    const leafStats = buildLeafFreq(cart, XtrS, ytr, /*alpha=*/2);
  ...
  // choose best avg loss
  let bestW = 0.5, bestL = Infinity;
  weights.forEach((w,i)=>{ if (losses[i] < bestL){ bestL = losses[i]; bestW = w; } });
  // early-season floor: keep at least some logistic signal until we have > 4 weeks
  return bestW < 0.3 ? 0.3 : Number(bestW.toFixed(2));
}
...
(async function main(){
  ...
  console.log(`Train/predict through WEEK ${MAX_WEEK} (cap of env=${WEEK_ENV}, data+1=${maxWeekAvail+1}, sched=${maxSchedWeek})`);
  ...
  function forecastMissingForWeek(W, trainRows, testRows){
    // Build rows for scheduled games in W that are missing from 'testRows', using last known S2D snapshot per team
    ...
    // last known S2D per team (season <= W-1)
  ...
  function runWeek(W){
    ...
    cart.train(Xtr, ytr);
    const leafStats = buildLeafFreq(cart, Xtr, ytr, 2);

    // CV choose hybrid
    const wHybrid = chooseHybridCV(Xtr, ytr);
    ...
    // forecast the rest of week W
    const foreRows = forecastMissingForWeek(W, train, test);
  ...
  for (let W=2; W<=MAX_WEEK; W++){
    ...
    seasonSummary.weeks.push({ week: W, train_rows: results.filter(r=>!r.forecast).length, forecast: results.some(r=>r.forecast), hybrid_weight: wHybrid });
