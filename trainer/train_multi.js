...
(async function main() {
  console.log(`Rolling train for SEASON=${SEASON}, up to WEEK=${MAX_WEEK}`);
  ...
  for (let W=2; W<=MAX_WEEK; W++) {
    ...
    console.log(`W${String(W).padStart(2,"0")}: train=${train.length}, test=${test.length}`);
    if (!train.length || !test.length) {
      console.log(`W${W}: skipping (insufficient rows)`);
      continue;
    }
